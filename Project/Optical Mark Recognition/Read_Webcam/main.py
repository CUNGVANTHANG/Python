...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------
# VARIABLE
path = "input.jpg"  # Đường dẫn tới "input.jpg"
widthImg = 700  # Chiều dài ảnh 700px
heightImg = 700  # Chiều cao ảnh 700px
questions = 5  # 5 câu hỏi
choices = 5  # 5 lựa chọn
ans = [1, 2, 0, 1, 4]  # Đáp án
webcamFeed = True
...
while True:
    ...
    else:img = cv2.imread(path)  # Đọc file ảnh từ đường dẫn path
    # ---------------------------------------------------------------------------------------------------------------------------------------------------------
    # PREPROCESSING
    ...
    try:
        ...
        if biggestContour.size != 0 and gradePoints.size != 0:  # Kiểm tra xem hai mảng đường viền biggestContour và gradePoints có kích thước khác không bằng 0
            ...
            imgWrapColored = cv2.warpPerspective(img, matrix, (widthImg, heightImg))  # Biến đổi góc nhìn, hình ảnh kết quả lưu vào imWrapColored
            ...
            imgGradeDisplay = cv2.warpPerspective(img, matrixG, (325, 150))  # Biến đổi góc nhìn, hình ảnh kết quả lưu vào imGradeDisplay
            ...
            for x in range(0, questions):
                arr = myPixelVal[x]
                # print("arr", arr)
                myIndexVal = np.where(arr == np.amax(arr))  # Tìm vị trí index của giá trị lớn nhất trong mảng arr
                # print(myIndexVal[0])
            ...
            invMatrix = cv2.getPerspectiveTransform(pt2G, pt1G)  # Tính ma trận biến đổi góc nhìn
    ...
    if cv2.waitKey(1) & 0xFF == ord('s'):
        ...
        cv2.putText(imgStacked, "Scan Saved", (int(imgStacked.shape[1] / 2) - 200, int(imgStacked.shaqqqpe[0] / 2)), cv2.FONT_HERSHEY_DUPLEX, 3, (0, 0, 255), 5, cv2.LINE_AA)
